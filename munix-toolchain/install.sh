...
echo "Building a toolchain with a sysroot of $HDD_SYSROOT with host binaries in $PREFIX targeting $TARGET"
...
pushd build
    if $BUILD_BINUTILS; then
        ...
        pushd binutils
            $DIR/tarballs/binutils-2.22/configure --target=$TARGET --prefix=$PREFIX --with-sysroot=$HDD_SYSROOT --disable-werror || bail
            make || bail
            make install || bail
        popd
    fi
    ...
    if $BUILD_GCC; then
        ...
        pushd gcc
            $DIR/tarballs/gcc-4.6.4/configure --target=$TARGET --prefix=$PREFIX --with-sysroot=$HDD_SYSROOT --with-build-sysroot=$HDD_SYSROOT --with-native-system-header-dir=$HDD_SYSROOT --disable-nls --enable-languages=c,c++ --disable-libssp --with-newlib || bail
            make all-gcc || bail
    ...
    if $BUILD_NEWLIB; then
        ...
        pushd $DIR/tarballs/newlib-1.19.0/newlib/libc/sys
            ...
            pushd munix
                ...
                cp crt0.o ../
                cp crt0.o /tmp/__munix_crt0.o
                cp crti.o ../
                cp crti.o /tmp/__munix_crti.o
                cp crtn.o ../
                cp crtn.o /tmp/__munix_crtn.o
            popd
        popd
        pushd newlib
            mkdir -p $TARGET/newlib/libc/sys
            cp /tmp/__munix_crt0.o $TARGET/newlib/libc/sys/crt0.o
            rm /tmp/__munix_crt0.o
            cp /tmp/__munix_crti.o $TARGET/newlib/libc/sys/crti.o
            rm /tmp/__munix_crti.o
            cp /tmp/__munix_crtn.o $TARGET/newlib/libc/sys/crtn.o
            rm /tmp/__munix_crtn.o
            ...
            sed -s 's/prefix}\/i686-munix/prefix}/' Makefile > Makefile.tmp
            mv Makefile.tmp Makefile
            make || bail
            make DESTDIR=$HDD_SYSROOT install || bail
            cp -r $DIR/patches/newlib/include/* $HDD_SYSROOT/$VIRTPREFIX/include/
            cp $TARGET/newlib/libc/sys/crt0.o $HDD_SYSROOT/$VIRTPREFIX/lib/
            cp $TARGET/newlib/libc/sys/crti.o $HDD_SYSROOT/$VIRTPREFIX/lib/
            cp $TARGET/newlib/libc/sys/crtn.o $HDD_SYSROOT/$VIRTPREFIX/lib/
        popd
    fi
    ...
    if $BUILD_ZLIB; then
        ...
        pushd $DIR/tarballs/zlib*
            CC=i686-munix-gcc ./configure --static --prefix=$VIRTPREFIX || bail
            make || bail
            make DESTDIR=$HDD_SYSROOT install || bail
        popd
    fi
    ...
    if $BUILD_NCURSES; then
        ...
        pushd ncurses
            ...
            make || bail
            make DESTDIR=$HDD_SYSROOT install || bail
            cp $DIR/../util/munix.tic $HDD_SYSROOT/$VIRTPREFIX/share/terminfo/t/munix
            cp $DIR/../util/munix-vga.tic $HDD_SYSROOT/$VIRTPREFIX/share/terminfo/t/munix-vga
        popd
    fi
    ...
    if $BUILD_VIM; then
        pushd $DIR/tarballs/vim73
            make distclean
            ac_cv_sizeof_int=4 vim_cv_getcwd_broken=no vim_cv_memmove_handles_overlap=yes vim_cv_stat_ignores_slash=no vim_cv_tgetent=zero vim_cv_terminfo=yes vim_cv_toupper_broken=no vim_cv_tty_group=world ./configure --host=$TARGET --target=$TARGET --with-sysroot=$HDD_SYSROOT --prefix=$VIRTPREFIX --with-tlib=ncurses --enable-gui=no --disable-gtktest --disable-xim --with-features=normal --disable-gpm --without-x --disable-netbeans --enable-multibyte
            make || bail
            make DESTDIR=$HDD_SYSROOT install || bail
        popd
    fi

    pushd $HDD_SYSROOT/usr/bin || bail
        $TARGET-strip *
    popd
